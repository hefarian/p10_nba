...
try:
    ...
    logging.info("PyMuPDF + pytesseract disponibles pour l'OCR.")
except ImportError as e:
    logging.warning(f"Modules OCR (PyMuPDF, Pillow, pytesseract) non installés: {e}. L'OCR ne sera pas disponible.")
    fitz = None
    Image = None
    pytesseract = None
...
def extract_text_from_pdf_with_ocr(file_path: str) -> Optional[str]:
    """Extrait le texte d'un fichier PDF en utilisant l'OCR (pytesseract)."""
    if not fitz_available or not pytesseract_available:
        logging.warning("Modules OCR non disponibles. Impossible d'effectuer l'OCR.")
        return None
    ...
    try:
        ...
        for page_num in tqdm(range(len(doc)), desc=f"OCR de {os.path.basename(file_path)}"):
            ...
            try:
                ...
                logging.error(f"Erreur OCR page {page_num + 1} de {file_path}: {ocr_e}")
                continue
        doc.close()
        ...
        logging.error(f"Erreur lors du traitement OCR du PDF {file_path}: {e}")
        return None
